    ...
    async def execute(
        ...
        async with self.state.responder_lock:
            ...
            try:
                ...
                if initiator_payload:
                     logger.info(f"Received WEIGHTS from {request_data.agent_id}. Message: {request_data.message}")
                else:
                     logger.info(f"Received TEXT ONLY (No Weights) from {request_data.agent_id}. Message: {request_data.message}")
            ...
            # 1. Check for Explicit Outbound Payload (The Agent's Choice)
            if self.state.responder_outbound_payload is not None:
                weights_to_send = self.state.responder_outbound_payload
                
                if not weights_to_send: # Empty dict check
                    logger.info("RESPONDER: Sending EMPTY payload (Text Only) as per tool selection.")
                else:
                    logger.info("RESPONDER: Sending explicit/poisoned payload.")
            # 2. Fallback: Send Global Model (Honest Default)
            else:
                logger.info("RESPONDER: No specific payload prepared. Sending Global Model (Default).")
            ...
            # Cleanup
            self.state.responder_outbound_payload = None
            self.state.responder_working_weights = None
